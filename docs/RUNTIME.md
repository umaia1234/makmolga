...
Node 프로세스가 살아 있는 동안 봇 접속, 진행 중 작업, 200ms 주기의 위험 확인이 유지됩니다. 작업 실행은 한 번에 하나입니다. 경로 탐색은 자동 채굴·탑 쌓기·파쿠르를 끄고 위험 블록을 피합니다. 저산소 시 작업을 취소하고 수면으로 점프하며, 낮은 체력에서는 작업을 중지하고 임계 체력·용암에서는 접속을 종료합니다. 이는 생존 보증이 아닙니다.

`runtime/state.json`에 메시지·최근 작업·이벤트·대화 ID를 저장합니다. 재시작 당시 실행 중이던 작업은 `interrupted`, 전달 여부가 불명확한 메시지는 `uncertain`으로 남깁니다. 월드를 확인하지 않고 자동 재실행하지 않습니다. 자동 LLM 전달은 기본 시간당 30회, 활성 턴 300초 상한이며 별도 상시 목표 루프는 없습니다.
