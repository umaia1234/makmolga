...
const files = [];
for (const dir of ['src', 'scripts', 'test']) if (fs.existsSync(path.join(ROOT, dir))) for (const file of fs.readdirSync(path.join(ROOT, dir))) if (file.endsWith('.mjs')) files.push(path.join(ROOT, dir, file));
