...
자율 모드를 켜면 캐릭터가 먼저 말을 걸고 관심사에 맞는 작은 일을 고르며, 캐릭터·월드별 계획을 저장합니다. 일상적인 부탁은 가끔 성격대로 미루거나 거절할 수 있습니다. “지금은 꼭 해 주세요”는 역할극 거절을 건너뜁니다. “멈춰”와 “자율 모드 꺼”는 LLM 응답을 기다리지 않고 처리합니다.

봇을 불러온 Minecraft 클라이언트가 앞·오른쪽·뒤·왼쪽·위·아래를 실제 렌더링해 LLM에 이미지로 전달합니다. 기본 촬영 간격은 45초, 계획 간격은 90초이며 자율 호출은 시간당 최대 24회입니다. 사용자 요청 한도와 별도입니다. 봇이 사용자 시야 범위에서 벗어나거나 게임을 종료하면 이미지 없이 블록·좌표·인벤토리 관찰을 사용합니다. 기존 배포 ZIP은 v0.2.1이므로 새 기능은 소스 빌드가 필요합니다. [설정·명령·검증 범위](docs/AUTONOMY.md)
