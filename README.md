...
[설치 파일](https://github.com/umaia1234/makmolga/releases/latest) · [빠른 시작](#빠른-시작) · [캐릭터 선택창](docs/CHARACTER-SELECTOR.md) · [로드맵](docs/ROADMAP.md)

</div>

![실제 Minecraft 26.2의 캐릭터 선택 화면](docs/images/character-selector.png)

**맠몰가**는 마인크래프트 안에서 함께할 LLM 동료를 만드는 프로젝트입니다. 현재 **v0.2**에는 네이티브 캐릭터 선택창, 별도 플레이어를 제어하는 Mineflayer 실행기, MCP 도구 15개, Codex App Server 대화 연결이 있습니다. 완전몰입형 가상현실은 프로젝트의 장기 목표이며, 현재 버전에 VR 기기 연동이나 감각 입출력 기능은 없습니다.

## 먼저, 함께할 친구를 고릅니다

| 캐릭터 | 성격 |
|---|---|
| **얀로롱** | 이론부터 확인하는 작은 교수님. 결국 함께 수습합니다. |
| **지피짱** | 차분하게 진단하고, 다정하게 실행을 독려합니다. |
| **도로롱** | 능청스러운 변명이 금세 들통나는 장난꾸러기입니다. |
...
- **게임 안의 선택창:** 원본 64×64 스킨을 실제 3D 플레이어 모델로 미리 봅니다. 서버와 LLM 없이도 사용할 수 있습니다.
...
- **캐릭터 성격:** 선택한 페르소나를 다음 LLM 대화에 참고 자료로 전달합니다. 사용자 원문과 존댓말·중지 규칙을 유지합니다.
...
| `character-pack/` | 다섯 캐릭터의 원본 스킨·성격과 표시용 목록 |
...
v0.2 로컬 검증에서 **Node 27개 + Java 6개 테스트**가 통과했습니다. 실제 Minecraft에서 새 월드·기존 월드의 자동 표시, H 키, 메뉴 버튼, 한글 툴팁, GUI 배율 2·3, 선택값의 로컬 실행기 전달을 확인했습니다. ViaProxy가 없는 환경에서는 해당 프로토콜 검사를 건너뛰므로 전체 검증에는 먼저 `npm run setup -- --download-proxy`를 실행합니다.
