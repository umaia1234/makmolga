...
**맠몰가**는 마인크래프트 안에서 함께할 LLM 동료를 만드는 프로젝트입니다. 현재 소스 **v0.3.0**에는 네이티브 캐릭터 선택창, 별도 플레이어를 제어하는 Mineflayer 실행기, MCP 도구 17개, Codex App Server 대화·자율 계획 연결이 있습니다. GitHub 배포 ZIP은 아직 v0.2.1입니다. 완전몰입형 가상현실은 프로젝트의 장기 목표이며, 현재 버전에 VR 기기 연동이나 감각 입출력 기능은 없습니다.
...
소스의 **v0.2.3 개발 빌드**에서는 게임 채팅도 `<얀로롱> 대사`처럼 캐릭터 이름으로 표시합니다. 다섯 캐릭터 모두 적용되며, 봇의 계정·UUID·인벤토리는 유지합니다. 모드가 없는 클라이언트에는 메시지 본문의 `[얀로롱]` 같은 이름 표시가 보입니다. 계정명 부분까지 바꾸려면 업데이트된 클라이언트 모드가 필요합니다.

자동 컨트롤러는 **캐릭터별 대화 기록**과 명시적인 이름·성격·말투 지시를 사용합니다. 다른 캐릭터로 전환하면 그 캐릭터의 대화를 만들거나 재개하며, 공통 사용자 메시지 기록과 실제 월드 상태는 유지합니다. 도로롱의 도로 소리 제한, 얀로롱의 매 채팅 LLM·JEPA·밈, 다른 캐릭터 이름으로 자기소개하는 오류는 출력 단계에서도 검사합니다. 작업 중 짧은 중간 대화는 각자의 말투를 따릅니다. [변경과 검증 범위](docs/CHAT-AND-PERSONAS.md)

## 먼저 움직이는 동료 (v0.3.0 소스)

함께 놀고 대화하는 동료를 지향합니다. 자율 모드에서는 먼저 말을 걸거나 놀 거리를 찾고, 부탁을 성격대로 미루거나 다른 놀이를 제안할 수도 있습니다. 거절 확률·정해진 대화 간격·의무적인 계획 보고는 없습니다. 일반 채팅은 진행 중인 게임 행동을 끊지 않으며, “멈춰”와 “자율 모드 꺼”는 즉시 처리합니다. 기억할 만한 목표와 사실은 캐릭터·월드별 메모로 남길 수 있습니다.

봇을 불러온 Minecraft 클라이언트가 앞·오른쪽·뒤·왼쪽·위·아래를 실제 렌더링해 LLM에 이미지로 전달합니다. 기본 촬영 간격은 45초, 계획 간격은 90초이며 자율 호출은 시간당 최대 24회입니다. 사용자 대화 전달에는 기본적으로 별도 상한을 걸지 않습니다. Codex 계정의 사용량 한도는 적용됩니다. 봇이 사용자 시야 범위에서 벗어나거나 게임을 종료하면 이미지 없이 블록·좌표·인벤토리 관찰을 사용합니다. 기존 배포 ZIP은 v0.2.1이므로 새 기능은 소스 빌드가 필요합니다. [설정·명령·검증 범위](docs/AUTONOMY.md)
...
준비가 끝나면 Codex 대화의 답변이 끝나도 **로컬 실행기가 게임 채팅을 기다립니다.** PC가 켜져 있고 절전 상태가 아니어야 합니다. 자율 모드가 꺼져 있으면 게임 요청이 없을 때 LLM 턴을 생성하지 않습니다. 자율 모드를 켜면 별도 호출 한도 안에서 계획을 주기적으로 갱신합니다. 이 기능은 로그인 시 자동 실행이나 주기적인 예약 작업을 등록하지 않습니다.
...
| `controller_rate_limited` | 직접 설정한 `controller.maxTurnsPerHour` 상한에 도달했습니다. 기본값 `null`은 실행기의 별도 대화 상한을 끕니다. |
