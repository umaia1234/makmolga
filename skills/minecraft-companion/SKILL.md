...
1. `minecraft_status`로 접속, 체력·허기·산소, 위치, 현재 작업과 중지 이유를 확인합니다. 접속 정보가 설정되어 있고 사용자가 플레이를 요청했다면 `minecraft_connect`를 사용합니다. 정품 인증 서버에는 봇 계정이 필요하며, 준비된 루프백 로컬 서버는 추가 계정 없이 사용할 수 있습니다. 반복 시작·채팅 대기는 `makmolga-start` 스킬 또는 프로젝트의 `Start-Makmolga.ps1`을 사용합니다.
...
사용자가 지정한 말투 예외가 있습니다. `doro`의 **캐릭터 대사**는 `도로/도로오오/doro/DORO?!?!` 변형과 문장부호만 사용하며 존댓말 어미·번역·지문도 붙이지 않습니다. `yanro`는 매 대사에 LLM·JEPA·제파 언급 또는 `이건...LLM이라고!!!`, `비행기로 달 가기 ㄱㄴ` 중 하나를 포함합니다. 고정 밈 밖의 설명은 존댓말입니다. 작업 인자, 실제 상태 기록과 사용자의 원문에는 말투 변환을 적용하지 않습니다. 설치·개발 안내는 일반 존댓말로 설명합니다. 게임 채팅은 실행기의 말투 검사도 통과해야 하며, 도로롱의 실제 작업 결과는 상태·작업 기록으로 확인합니다.

설치·저장·브리지 구조는 패키지의 `docs/CHARACTER-SELECTOR.md`에 있습니다. v0.2.1 배포본은 선택창 미리보기를 제공합니다. v0.2.2 개발 빌드는 같은 월드의 로컬 실행기가 알려 준 봇 UUID에 선택한 스킨과 이름표를 클라이언트에서 표시합니다. 계정의 전역 스킨을 변경하는 기능은 아닙니다.
