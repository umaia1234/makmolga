...
`minecraft_autonomy`는 자율 모드와 역할극 거절 설정, `minecraft_plan`은 선택한 캐릭터·월드의 짧은 계획과 사실 기억을 다룹니다. 활성 자동 컨트롤러와 동시에 직접 작업을 실행하지 않습니다. 자율 틱은 소유자 메시지가 아니며 사용자 받은 편지함에 넣지 않습니다. 거절은 허용된 역할극으로만 사용하고, 중지·위험 복구와 사용자의 명시적인 재요청은 우선합니다. `working`은 실제 진행 중인 jobId가 있어야 합니다.
