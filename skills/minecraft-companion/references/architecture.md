...
flowchart TD
  ...
  Guard[200ms 상태 확인] -->|관찰 · 자동 식사 · 선택적 보호| Bot
...
| `src/voice.mjs` | 도로롱의 소리만 허용, 얀로롱의 LLM·JEPA·밈 필수 대사 | 도로롱 규칙 위반은 `도로?`, 얀로롱 주제 누락은 고정 밈을 앞에 추가; 실제 작업 기록은 보존 |
...
- `minecraft`: 최종 서버 주소, 목표·클라이언트 버전, 봇 식별자, 접속 방식. `autoConnect`는 시작 시 자동 접속이고 `reconnectAttempts`는 연결이 끊긴 경우의 제한된 재시도 횟수입니다. 30초 동안 안정적으로 접속하면 횟수를 초기화합니다. 수동 접속 종료·사망에서는 자동 재접속하지 않습니다. 임계 체력에 따른 접속 종료는 `safety.protectiveStops=true`일 때 적용합니다.
...
- `safety`: 체력/산소 관찰 기준, 자동 식사 기준, 한 이동의 최대 거리, `return_home` 목적지. `protectiveStops` 기본값은 `false`이며, `true`로 설정하면 저산소 자동 수영·저체력 정지·임계 체력/용암 로그아웃을 켭니다. `home`을 설정한다고 자율 순찰이나 자동 귀환 목표가 생기지는 않습니다.
- `controller`: 자동 전달 사용 여부, App Server 통신 방식, Codex 실행 경로/로컬 WebSocket URL, 이 패키지가 만든 대화 ID, 선택적 모델, 게임 답변 중계, 전달/활성 턴 상한. `maxTurnsPerHour`는 실제 구현상 새 턴과 steer를 합친 메시지 전달 횟수를 셉니다. 기본값 `null`은 이 실행기의 별도 대화 상한을 끄며, 배경 자율 호출에는 독립된 한도가 있습니다. `idleTimeoutSeconds`는 마지막으로 전달된 요청 이후의 활성 턴 제한 시간입니다.
...
운영체제 입력기를 사용하지 않으므로 사용자님의 키보드와 마우스를 점유하지 않습니다. 별도 게임 창은 필수가 아닙니다. 서버가 보낸 블록/엔티티/인벤토리 데이터를 사용하며, v0.3.0부터 로드된 봇 위치의 여섯 방향 이미지를 주기적으로 모델에 전달할 수 있습니다. 연속 영상 스트리밍은 아닙니다.
