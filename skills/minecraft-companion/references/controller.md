...
- 사용자 대화 전달 상한은 기본 `null`로 꺼져 있습니다. `controller.maxTurnsPerHour`를 직접 지정하면 `turn/start`와 `turn/steer`를 함께 세며, 상한에 도달한 메시지는 시간 창이 지나면 전달합니다. 배경 자율 호출 한도와 Codex 계정 사용량 한도는 별도로 적용됩니다.
